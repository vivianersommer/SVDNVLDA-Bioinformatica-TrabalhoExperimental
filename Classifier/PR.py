...
while counter0 < 10:
    ...
    RAPName = '第' + str(counter0+1) + '次Score.csv'
    RAPNameProb = '第' + str(counter0+1) + '次Prob.csv'
    ...
    # 弧线
    plt.plot(recall, precision, lw=1.5, alpha=0.8, color='red',
...
plt.ylim([0, 1])
plt.xlim([0, 1])
plt.title('Precision-Recall curve: AUPR={0:0.4f}'.format(mean_RPs))
...
plt.savefig('PR-10fold.svg')
plt.savefig('PR-10fold.tif')
plt.show()
