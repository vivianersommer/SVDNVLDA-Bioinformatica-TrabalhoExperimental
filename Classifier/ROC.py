...
storFile(MyNew, '十折的表（由保存值生成）.csv')
#print(MyNew)
...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
storFile(FprAndTpr, 'FprAndTpr.csv')
...
plt.title('Receiver Operating Characteristic: AUC={0:0.4f}'.format(mean_auc))
plt.legend()
plt.grid(linestyle='-')
# # 保存图片
plt.savefig('ROC-10fold.svg')
plt.savefig('ROC-10fold.tif')
plt.show()